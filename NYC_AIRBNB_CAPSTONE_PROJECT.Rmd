...
```{r, echo=FALSE}
RMSE_results <- bind_rows(RMSE_results,
                          data_frame(method="XGBOOST LINEAR MODEL",RMSE = RMSE(exp(Model_3_predict)-1,test_set$data.price) ))
```
...
The results obtained by running the above three models by performing training on **train_set** dataset and further testing on the **test_set ** dataset are as follows :
...
```{r}
RMSE(exp(VALIDATION_predict)-1,validation_set$data.price)
```

The high RMSE Value are due to the Outliers present in the form of High Price Houses in the dataset.
Thus this makes it necessary to calculate an RMSE without taking into consideration the outliers for an estimate of RMSE error of the model on dataset without the outliers.

#### Calculating the RMSE for the validation data_Set excluding the top 10 percent values.
...
```{r,warning=FALSE}
tibble(method = "XGBLINEAR BOOST ON VALIDATION_SET RMSE AFTER FILTERING OUT OUTLIERS"
...
tibble(method = "XGBLINEAR BOOST ON VALIDATION_SET "
       , RMSE =RMSE(exp(VALIDATION_predict)-1,validation_set$data.price))%>%knitr::kable()

```

Also the further analysis of the final model after removing outliers for the **validation_set**, provides a conclusion that the model fits in the better way for the housings after removing the top 5 percent outliers. This is evident by the RMSE value :
